...
// todo: make specific oauth middleware as custom auth middleware is invalidating session made below
export const GET = async() => {
    ...
    try {
        ...
        const url = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${process.env.GOOGLE_CLIENT_ID}&nonce=${nonce}&scope=${scopes.join('%20')}&redirect_uri=${encodedRedirectUri}&state=${createdSession.data.state}&response_type=code&access_type=offline`;
